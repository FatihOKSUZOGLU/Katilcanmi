...
class _PaymentScreenState extends State<PaymentScreen> {
          ...
          child: Column(
            ...
            children: [
              ...
              DropdownButtonFormField<PaymentType>(
                ...
                items: PaymentType.values.map((type) {
                  return DropdownMenuItem(
                    value: type,
                    child: Text(
                        type == PaymentType.artan ? "Artan" : "Sabit"),
                  );
                }).toList(),
                onChanged: (val) {
                  vm.paymentType = val!;
              ...
              ElevatedButton(
                onPressed: () {
                  vm.katilimOrani =
                      double.tryParse(katilimController.text) ?? 0;

                  vm.katilimTaksitSayisi = selectedKatilimTaksit;

                  vm.calculate(
                    teslimAy:
                        int.tryParse(teslimController.text) ?? 0,
                    toplamAy:
                        int.tryParse(toplamController.text) ?? 0,
                    pesinat:
                        double.tryParse(pesinatController.text) ?? 0,
                    anaPara:
                        double.tryParse(anaParaController.text) ?? 0,
                  );
                },
              ...
              Text(
                "Önerilen Peşinat: ${vm.suggestedPesinat}",
              ),
              Text(
                "İlk Taksit: ${vm.ilkTaksit}",
              ),
              ...
              if (vm.rows.isEmpty)
                const Center(
                  child: Text("Henüz hesaplama yapılmadı"),
                )
              else
                ListView.builder(
                  shrinkWrap: true,
  ...
  Widget _buildValidation(PaymentViewModel vm) {
    final value =
        double.tryParse(vm.validationResult) ?? 0;
